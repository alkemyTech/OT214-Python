...
with DAG(
    ...
    def send():
        ...
        def send_s3():
            """
            send txt files to s3 bucket with boto3 library.

            """
            bucket = config('_AWS_BUCKET')
            public = config('_AWS_PUBLIC_KEY')
            secret = config('_AWS_SECRET_KEY')
            path_txt_interamericana = (f'{path}/files/'
                                       'universidad_abierta_'
                                       'interamericana.txt')

            path_txt_pampa = (f'{path}/files/universidad_'
                              'nacional_de_la_pampa.txt')

            connect = boto3.client('s3', aws_access_key_id=public,
                                   aws_secret_access_key=secret)

            with open(path_txt_pampa, 'rb') as file1:
                connect.upload_fileobj(file1, bucket, 'universidad_nacional_'
                                       'de_la_pampa.txt')

            lg_send.info('upload universidad_nacional_'
                         'de_la_pampa.txt comlpeted.')

            with open(path_txt_interamericana, 'rb') as file2:
                connect.upload_fileobj(file2, bucket, 'universidad_abierta_'
                                       'interamericana.txt')

            lg_send.info('upload universidad_abierta_interamericana.txt'
                         ' completed.')
